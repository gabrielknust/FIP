...
<script>
   $(function(){
    ...
    $.each(ocorrencia,function(i,item){
      $("table").append($("<tbody />").append($("<tr />").attr("class",item.classificaUrgencia).append($("<td />").text(item.cep)).append($("<td />").text(item.bairro)).append($("<td />").text(item.rua)).append($("<td />").text(item.referencia))));
    });
  });
</script>
